...
static Loko4LegState leg4_state = {
    ...
    .old_R2 = 0,
    .recharge = 0,
    .coordinate_RF = {NEUTRAL_REACH_MM, 0.0f, LOKO_FRONT_RAISE_MM},
    .coordinate_LF = {NEUTRAL_REACH_MM, 0.0f, LOKO_FRONT_RAISE_MM}
};
...
static void state_walk_4_legs(LokoState *st, const LokoInput *in, float dt)
{
    ...
    if (loko_pressed(&st->pad, BTN_CROSS)) {
    ...
    int hit_active = loko_held(&st->pad, BTN_L2) || loko_held(&st->pad, BTN_R2);
...
static const StateHandler state_table[LOKO_STATE_COUNT] = {
    [LOKO_UN_ARMED]        = state_un_armed,
    [LOKO_STAND]           = state_stand,
    [LOKO_WALK]            = state_walk,
    [LOKO_WALK_4_LEGS]     = state_walk_4_legs,
    [LOKO_STAND_4_LEGS]    = state_stand_4_legs,
    [LOKO_ROTATE_IN_PLACE] = state_rotate_in_place,
    [LOKO_DANCING]         = state_dancing,
};
